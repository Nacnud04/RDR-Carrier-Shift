...
model_dirs = ("./", "./")
models = ("models/hm_noise_050.keras", "models/hl_noise_050.keras")
...
datasets = [io.dataset(dir='../data/sizes', pair=p) for p in ((0, 1), (0, 2))]
...
sc = ax[0, 0].imshow(sources[i], cmap="gray")
ax[0, 0].set_title("HF Source")
ax[0, 0].set_ylabel("Target")

tg_mf = ax[0, 1].imshow(targets1[i], cmap="gray")
ax[0, 1].set_title("MF")

tg_lf = ax[0, 2].imshow(targets2[i], cmap="gray")
ax[0, 2].set_title("LF")

ax[1, 0].axis('off')

pd_mf = ax[1, 1].imshow(preds[0][i], cmap="gray")
ax[1, 1].set_ylabel("Result")

pd_lf = ax[1, 2].imshow(preds[1][i], cmap="gray")
...
def update(i):
    ...
    suptitle.set_text("Example: {i:03d}")
