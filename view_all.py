...
parser.add_argument("-i", "--index", default=0, type=int, help="Index to start at")
parser.add_argument("-c", "--count", default=1, type=int, help="How many results to show after start index")
args = parser.parse_args()

model_dirs = ("high_med", "high_low")

print(f"[yellow]Loading models...[/yellow]")
models = [tf.keras.models.load_model(f'{md}/carrier_shift.keras') for md in model_dirs]
print(f"[bold green]Loaded models![/bold green]")

print(f"[yellow]Loading testing datasets...[/yellow]")
datasets = [io.dataset(dir='data/random_planes/valid', pair=p) for p in ((0, 1), (0, 2))]
...
for i in [args.index + c for c in range(args.count)]:
