...
dataset = io.dataset(dir='../data/noise', pair=(0, 2))
hl_train_dataset = dataset.tf_dataset(batch_size)
dataset = io.dataset(dir='../data/noise/valid', pair=(0, 2))
hl_valid_dataset = dataset.tf_dataset(batch_size)

# --- LOAD HM DATASETS ---
yprint("Loading HM datasets...")
dataset = io.dataset(dir='../data/noise', pair=(0, 1))
hm_train_dataset = dataset.tf_dataset(batch_size)
dataset = io.dataset(dir='../data/noise/valid', pair=(0, 1))
...
shutil.copyfile("../camp_hl/models/camp_hl_100.keras", "./hl_noise.keras")
shutil.copyfile("../camp_hm/models/camp_hm_100.keras", "./hm_noise.keras")

# --- LOAD HL MODEL ---
import keras

yprint(f"Loading and compiling model...")
hl_model = keras.saving.load_model("./hl_noise.keras")
hl_model.compile(optimizer=keras.optimizers.Adam(1e-4), loss="mean_squared_error")

callbacks = [
    keras.callbacks.ModelCheckpoint(
        filepath="models/hl_noise_{epoch:03d}.keras",
...
# --- LOAD HM MODEL
yprint(f"Loading and compiling model...")
hm_model = keras.saving.load_model("./hm_noise.keras")
hm_model.compile(optimizer=keras.optimizers.Adam(1e-4), loss="mean_squared_error")

callbacks = [
    keras.callbacks.ModelCheckpoint(
        filepath="models/hm_noise_{epoch:03d}.keras",
        save_best_only=False,
        save_freq='epoch'
    )
]
