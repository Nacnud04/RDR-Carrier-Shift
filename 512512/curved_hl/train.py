...
dataset = io.dataset(dir='../data/curved', pair=(0, 2))
...
shutil.copyfile("../planar_hl/planar_hl.keras", "./curved_hl.keras")

import keras

# load model
yprint(f"Loading and compiling model...")
model = keras.saving.load_model("./curved_hl.keras")
model.compile(optimizer=keras.optimizers.Adam(1e-4), loss="mean_squared_error")

callbacks = [
    keras.callbacks.ModelCheckpoint(
        filepath="models/curved_hl_{epoch:03d}.keras",
